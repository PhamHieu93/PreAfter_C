...
unsigned char current_dir = 1;    /* direction is forward at time 0s _ west */
...
uint16_t stepper(void)
{
    ...
    while (x < 3)
    {
        if (neighbour_val[0] > neighbour_val[x + 1])
        {
            ...
            min = pow(2, ( 2 * x2 )) ;
            /*
             *  first, neighbor_val [4] have the value of 1 ( direction of North)
...
int main(void)
{
  /* USER CODE BEGIN 1 */
	...
	HAL_Delay(500);
    ford_ward();
    if (current_dir == 1) current_cell = current_cell + 6;
    if (current_dir == 4) current_cell = current_cell + 1;
    if (current_dir == 16) current_cell = current_cell - 6;
    if (current_dir == 64) current_cell = current_cell - 1;
